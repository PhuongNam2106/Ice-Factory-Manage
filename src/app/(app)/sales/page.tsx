            ...
            {sales.map((sale) => (
              <li className="flex flex-col gap-3 p-5 transition-colors hover:bg-slate-50/80 sm:flex-row sm:items-center sm:justify-between" key={sale.id}>
                ...
                <div className="space-y-2 text-right sm:self-center">
                  ...
                  <p className="text-[11px] text-slate-400">{new Date(sale.occurredAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Bangkok' })}</p>
                  {sale.status === 'active' && (user.role === 'manager' || sale.createdBy === user.id) ? <div className="flex flex-wrap justify-end gap-2"><CorrectOccurredAtDialog entityId={sale.id} entityType="sale" label="đơn bán" occurredAt={sale.occurredAt} version={sale.version} /><CancelDocumentDialog entityId={sale.id} entityType="sale" label="đơn bán" version={sale.version} /></div> : null}
                </div>
              </li>
            ))}
          </ul>
