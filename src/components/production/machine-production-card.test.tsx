...
describe('MachineProductionCard', () => {
  it('only enables start while a machine is stopped', () => {
    render(<MachineProductionCard isManager={false} locked={false} machine={stopped} managerWritable now={new Date('2026-09-05T14:00:00Z')} productionDate="2026-09-05" productionEndsAt="2026-09-06T11:00:00Z" reminderMinutes={30} writable />)
  ...
  it('shows a zero-bag harvest and explains pending harvest blocking', () => {
    ...
    render(<MachineProductionCard isManager={false} locked={false} machine={machine} managerWritable now={new Date('2026-09-05T14:01:00Z')} productionDate="2026-09-05" productionEndsAt="2026-09-06T11:00:00Z" reminderMinutes={30} writable />)
    expect(screen.getByRole('button', { name: 'Xả đá' })).toBeDisabled()
    expect(screen.getByText(/Lần xả gần nhất chưa có số bao/)).toBeInTheDocument()
    expect(screen.queryByText('Xả đá · 0 bao')).not.toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Nhật ký máy' })).toHaveAttribute(
      'href',
      `/production/machines/${machine.id}?day=2026-09-05`,
    )
