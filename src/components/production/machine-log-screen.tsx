...
export function MachineLogScreen({ machine, productionDate, currentProductionDate, isManager, locked }: { machine: MachineProductionState; productionDate: string; currentProductionDate: string; isManager: boolean; locked: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {historical ? <div className="rounded-2xl border border-slate-300 bg-slate-100 p-4 text-sm font-bold text-slate-800">Bạn đang xem ngày cũ. Nhân viên chỉ được xem; quản lý có thể nhập bù, hiệu chỉnh hoặc xóa từ hành động mới nhất nếu ngày chưa khóa.</div> : null}
    <MachineProductionLog allowBackfill={historical} isManager={isManager} locked={locked} machine={machine} productionDate={productionDate} writable={writable} />
  </div>
}
