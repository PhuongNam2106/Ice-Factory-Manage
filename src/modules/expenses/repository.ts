...
export async function listExpenses(client: ExpenseClient, limit = 100): Promise<ExpenseItem[]> {
  const { data, error } = await client
    .from('expenses')
    .select(
      'id, operating_day, category_id, amount_vnd, payee, note, status, version, review_reason, created_by, occurred_at, created_at, reviewed_by, reviewed_at, expense_categories(name), expense_attachments(id, expense_id, original_name, content_type, size_bytes, created_at)',
    )
