...
test('runs one operating day from opening stock to locked report', async ({ page, context }) => {
  ...
  await page.goto('/production/new/batch')
  await page.locator('[name="startTime"]').fill('06:00')
  await page.locator('[name="endTime"]').fill('10:00')
  await page.locator('[name="goodBags"]').fill('50')
  ...
  await expect(page.getByText('Tồn hệ thống trước kiểm').locator('..')).toContainText('100 bao')
  await page.locator('[name="actualBags"]').fill('100')
  ...
  await page.getByRole('button', { name: 'Chọn từng mẻ' }).click()
  await expect(page.getByRole('button', { name: 'Chọn từng mẻ' })).toHaveCount(0)
